...
class CrashReporterApp(App):
    ...
    def build(self):
        ...
        lbl.bind(texture_size=lambda instance, val: setattr(instance, 'height', max(val[1], 1000)))
...
def _run_app():
    ...
    def write_crash_log(error_text: str):
        try:
            ...
            base_dir = app.user_data_dir if app and hasattr(app, 'user_data_dir') and app.user_data_dir else os.path.expanduser("~")
    ...
    class MusicPlayerApp(MDApp):
        ...
        def open_crash_log_viewer(self):
            ...
            btn_clear = Button(text="Clear Logs", background_color=(0.85, 0.25, 0.25, 1), size_hint=(0.5, 1))
            btn_clear.bind(on_release=_clear_logs)

            btn_close = Button(text="Close", background_color=(0.25, 0.25, 0.25, 1), size_hint=(0.5, 1))
        ...
        def play_track(self, track: dict, start_pos: float = 0.0):
            ...
            if thumb_url:
                ...
                threading.Thread(target=_download_artwork, name="ArtworkDownloadThread", daemon=True).start()

            def _fetch_audio():
                audio_path, duration = self.search_engine.prepare_audio_file(track.get('webpage_url', ''), track_id)
                if fetch_id != self._current_fetch_id:
                    return
                if audio_path:
...
if __name__ == '__main__':
    try:
        ...
        print(f"[FATAL LAUNCH ERROR]\n{err}")
        CrashReporterApp(err).run()